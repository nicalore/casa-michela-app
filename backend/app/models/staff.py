...
class Staff(Base):
    ...
    __table_args__ = (
        CheckConstraint("iban IS NULL OR length(iban) = 27", name="iban_length"),
        CheckConstraint("iban IS NULL OR iban = upper(iban)", name="iban_uppercase"),
        CheckConstraint(
    ...
    collaboration_type: Mapped[CollaborationTypeEnum] = mapped_column(
        SqlEnum(CollaborationTypeEnum, name="collaboration_type_enum"),
        nullable=False,
    )
