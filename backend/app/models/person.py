...
def _has_valid_tax_code_check_character(tax_code: str) -> bool:
    ...
    return (
        tax_code[-1]
        == _TAX_CODE_CHECK_CHARACTERS[total % len(_TAX_CODE_CHECK_CHARACTERS)]
    )
...
class Person(CreatedAtMixin, UpdatedAtMixin, Base):
    ...
    presences_booked: Mapped[list[Presence]] = relationship(
        back_populates="booker",
        foreign_keys="[Presence.booker_tax_code]",
        cascade="all, delete-orphan",
    )
