...
def _get_role_filter(role: str | None, select_stmt):
    ...
    if role == "administrator":
        return select_stmt.join(Staff, Member.tax_code == Staff.tax_code).join(Administrator, Staff.tax_code == Administrator.tax_code)
    elif role == "psychologist":
        return select_stmt.join(Staff, Member.tax_code == Staff.tax_code).join(Psychologist, Staff.tax_code == Psychologist.tax_code)
    elif role == "teacher":
        return select_stmt.join(Staff, Member.tax_code == Staff.tax_code).join(Teacher, Staff.tax_code == Teacher.tax_code)
    elif role == "student":
        return select_stmt.join(Student, Member.tax_code == Student.tax_code)
    elif role == "course_participant":
        return select_stmt.join(CourseParticipant, Member.tax_code == CourseParticipant.tax_code)
    
    return select_stmt

def _get_person_role_filter(role: str, select_stmt):
    stmt = select_stmt.join(Member, Person.tax_code == Member.tax_code)
    if role == "administrator":
        return stmt.join(Staff, Member.tax_code == Staff.tax_code).join(Administrator, Staff.tax_code == Administrator.tax_code)
    elif role == "psychologist":
        return stmt.join(Staff, Member.tax_code == Staff.tax_code).join(Psychologist, Staff.tax_code == Psychologist.tax_code)
    elif role == "teacher":
        return stmt.join(Staff, Member.tax_code == Staff.tax_code).join(Teacher, Staff.tax_code == Teacher.tax_code)
    elif role == "student":
        return stmt.join(Student, Member.tax_code == Student.tax_code)
    elif role == "course_participant":
        return stmt.join(CourseParticipant, Member.tax_code == CourseParticipant.tax_code)
    return stmt
...
async def _calculate_current_totals_dashboard(role: str | None, db: AsyncSession) -> CurrentTotalsResponse:
    ...
    m_curr_q = select(func.count(Membership.member_tax_code)).join(Member).where(Membership.year == c_year)
    m_curr_q = _get_role_filter(role, m_curr_q)
    m_curr = await db.scalar(m_curr_q) or 0

    c_curr_q = select(func.count(Membership.member_tax_code)).join(Member).where(and_(Membership.year == c_year, Member.collaborating_active == True))
    c_curr_q = _get_role_filter(role, c_curr_q)
    c_curr = await db.scalar(c_curr_q) or 0

    m_start_m_q = select(func.count(Membership.member_tax_code)).join(Member).where(and_(Membership.year == c_year, func.extract('month', Membership.start_date) < c_month))
    m_start_m_q = _get_role_filter(role, m_start_m_q)
    m_start_m = await db.scalar(m_start_m_q) or 0

    c_start_m_q = select(func.count(Membership.member_tax_code)).join(Member).where(and_(Membership.year == c_year, func.extract('month', Membership.start_date) < c_month, Member.collaborating_active == True))
    c_start_m_q = _get_role_filter(role, c_start_m_q)
    c_start_m = await db.scalar(c_start_m_q) or 0

    m_start_y_q = select(func.count(Membership.member_tax_code)).join(Member).where(Membership.year == c_year - 1)
    m_start_y_q = _get_role_filter(role, m_start_y_q)
    m_start_y = await db.scalar(m_start_y_q) or 0

    c_start_y_q = select(func.count(Membership.member_tax_code)).join(Member).where(and_(Membership.year == c_year - 1, Member.collaborating_active == True))
    ...
    if role is not None:
        gen_m_curr_q = select(func.count(Membership.member_tax_code)).join(Member).where(Membership.year == c_year)
        gen_m_curr = await db.scalar(gen_m_curr_q) or 0

        gen_c_curr_q = select(func.count(Membership.member_tax_code)).join(Member).where(and_(Membership.year == c_year, Member.collaborating_active == True))
...
async def _execute_members_trend(role: str | None, resolution: str, start_year: int | None, end_year: int | None, db: AsyncSession):
    if resolution == "month":
        query = select(Membership.year, func.extract('month', Membership.start_date).label('month'), func.count(Membership.member_tax_code).label("total")).join(Member).group_by(Membership.year, func.extract('month', Membership.start_date))
    else:
        query = select(Membership.year, func.count(Membership.member_tax_code).label("total")).join(Member).group_by(Membership.year)
    ...
    return [MemberTrendItem(year=r.year, month=int(r.month) if resolution == "month" and r.month else None, total_members=r.total) for r in result.all()]
...
async def _execute_collaborating_trend(role: str | None, resolution: str, start_year: int | None, end_year: int | None, db: AsyncSession):
    if resolution == "month":
        query = select(Membership.year, func.extract('month', Membership.start_date).label('month'), func.count(Membership.member_tax_code).label("total")).join(Member).where(Member.collaborating_active == True).group_by(Membership.year, func.extract('month', Membership.start_date))
    else:
        query = select(Membership.year, func.count(Membership.member_tax_code).label("total")).join(Member).where(Member.collaborating_active == True).group_by(Membership.year)
...
async def _execute_retention_rate(role: str | None, year: int, db: AsyncSession) -> RetentionRateItem:
    subq_prev_stmt = select(Membership.member_tax_code).join(Member).where(Membership.year == year - 1)
    ...
    if prev_count == 0:
        return RetentionRateItem(year=year, previous_year_members=0, retained_members=0, retention_rate_percentage=0.0)
        
    retained_query = select(func.count(Membership.member_tax_code)).join(Member).where(and_(Membership.year == year, Membership.member_tax_code.in_(select(subq_prev.c.member_tax_code))))
    ...
    return RetentionRateItem(year=year, previous_year_members=prev_count, retained_members=retained_count, retention_rate_percentage=round((retained_count / prev_count) * 100.0, 2))
...
async def _execute_collaborating_retention(role: str | None, year: int, month: int, db: AsyncSession) -> RetentionRateItem:
    ...
    subq_prev_stmt = select(Membership.member_tax_code).join(Member).where(and_(Membership.year == prev_y, func.extract('month', Membership.start_date) <= prev_m, Member.collaborating_active == True))
    subq_prev_stmt = _get_role_filter(role, subq_prev_stmt)
    subq_prev      = subq_prev_stmt.subquery()
    ...
    if prev_count == 0:
        return RetentionRateItem(year=year, month=month, previous_year_members=0, retained_members=0, retention_rate_percentage=0.0)
        
    retained_query = select(func.count(Membership.member_tax_code)).join(Member).where(and_(Membership.year == year, func.extract('month', Membership.start_date) <= month, Member.collaborating_active == True, Membership.member_tax_code.in_(select(subq_prev.c.member_tax_code))))
    retained_query = _get_role_filter(role, retained_query)
    retained_count_raw  = await db.scalar(retained_query)
    retained_count: int = int(retained_count_raw) if retained_count_raw is not None else 0
    
    return RetentionRateItem(year=year, month=month, previous_year_members=prev_count, retained_members=retained_count, retention_rate_percentage=round((retained_count / prev_count) * 100.0, 2))
...
async def get_general_members_trend(resolution: str = Query("year"), start_year: int | None = Query(None), end_year: int | None = Query(None), db: AsyncSession = Depends(get_db)):
...
async def get_general_collaborating_trend(resolution: str = Query("year"), start_year: int | None = Query(None), end_year: int | None = Query(None), db: AsyncSession = Depends(get_db)):
...
async def get_general_retention_rate(year: int = Query(...), db: AsyncSession = Depends(get_db)):
...
async def get_general_collaborating_retention(year: int = Query(...), month: int = Query(...), db: AsyncSession = Depends(get_db)):
...
async def get_role_current_totals(role: str = Query(...), db: AsyncSession = Depends(get_db)):
...
async def get_role_members_trend(role: str = Query(...), resolution: str = Query("year"), start_year: int | None = Query(None), end_year: int | None = Query(None), db: AsyncSession = Depends(get_db)):
...
async def get_role_collaborating_trend(role: str = Query(...), resolution: str = Query("year"), start_year: int | None = Query(None), end_year: int | None = Query(None), db: AsyncSession = Depends(get_db)):
...
async def get_role_retention_rate(role: str = Query(...), year: int = Query(...), db: AsyncSession = Depends(get_db)):
...
async def get_role_collaborating_retention(role: str = Query(...), year: int = Query(...), month: int = Query(...), db: AsyncSession = Depends(get_db)):
...
async def get_role_city_distribution(role: str = Query(...), db: AsyncSession = Depends(get_db)):
    query = select(Person.residence_city.label("city"), func.count(Person.tax_code).label("member_count")).group_by(Person.residence_city)
    query = _get_person_role_filter(role, query)
    query = query.order_by(func.count(Person.tax_code).desc())
    
    result = await db.execute(query)
    return [CityDistributionItem(city=r.city, count=r.member_count) for r in result.all()]

@router.get("/role/age-distribution", response_model=list[AgeDistributionItem])
async def get_role_age_distribution(role: str = Query(...), db: AsyncSession = Depends(get_db)):
    query = select(Person.birth_date)
    query = _get_person_role_filter(role, query)
    result = await db.execute(query)
    dates = result.scalars().all()
    
    buckets = {"< 11": 0, "11-14": 0, "15-18": 0, "19-25": 0, "26-35": 0, "36-50": 0, "> 50": 0}
    today = date.today()
    ...
    return [AgeDistributionItem(age_group=k, count=v) for k, v in buckets.items() if v > 0]
...
async def get_student_education_distribution(
    ...
    query = select(func.count(func.distinct(Student.tax_code)).label("student_count"))
    query = query.join(SchoolEnrollment, Student.tax_code == SchoolEnrollment.student_tax_code)
    ...
    if distribution_type == "school":
        ...
        ).join(School, SchoolStudyProgram.school_mechanographic_code == School.mechanographic_code)
    ...
    def _format_level(val: str) -> str:
        if val == "PRIMARY_SCHOOL": return "Scuola Primaria"
        if val == "MIDDLE_SCHOOL": return "Scuola Sec. I grado"
        if val == "HIGH_SCHOOL": return "Scuola Sec. II grado"
        return val

    items = []
    for r in result.all():
        lbl = _format_level(r.label) if distribution_type == "level" else r.label
        items.append(EducationDistributionItem(label=lbl, count=r.student_count))

    return items

@router.get("/teachers/subjects-statistics", response_model=TeacherSubjectsStatisticsResponse)
async def get_teacher_subjects_statistics(ranking_mode: str = Query("absolute"), db: AsyncSession = Depends(get_db)):
    #Comment:Isolate active teachers
    active_teachers_subq = select(Teacher.tax_code).join(Staff).join(Member).where(Member.collaborating_active == True).subquery()
    ...
    ).join(AssociationSubject, TeachingCompetence.association_subject_id == AssociationSubject.id)\
     .join(StudyProgram, TeachingCompetence.study_program_id == StudyProgram.id)\
     .where(TeachingCompetence.teacher_tax_code.in_(select(active_teachers_subq)))
    ...
    subj_counts = [{"name": subject_names[k], "count": len(t_set)} for k, t_set in subject_teachers.items()]
    ...
    for area, t_set in area_teachers.items():
        ...
        area_dist.append(AreaDistributionItem(area=area, count=count, percentage=round(perc, 1)))
...
async def get_course_participant_distribution(db: AsyncSession = Depends(get_db)):
    #Comment:Extract course distribution for course participants
    query = select(
        CourseParticipant.course_type.label("label"), 
        func.count(CourseParticipant.tax_code).label("count")
    ).group_by(CourseParticipant.course_type)\
     .order_by(func.count(CourseParticipant.tax_code).desc())
    
    result = await db.execute(query)
    return [CourseDistributionItem(label=r.label, count=r.count) for r in result.all()]