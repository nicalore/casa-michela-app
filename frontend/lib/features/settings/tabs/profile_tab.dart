...
class _ProfileTabState extends State<ProfileTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isStaff =
        rawRoles.contains('AMMINISTRATORE') ||
          ...
          child: Column(
            ...
            children: [
              ...
              if (_selectedSubTab == 0) ...[
                      ...
                      Expanded(
                        child: _ProfileSectionCard(
                          ...
                          leadingIcon: _ProfileAvatar(
                            ...
                            onImageUpdated: () =>
                                _fetchProfile(isInitialLoad: false),
                          ),
                          rows: [
                ...
                if (rawRoles.contains('AMMINISTRATORE') ||
                    rawRoles.contains('ADMIN')) ...[
                  const SizedBox(height: 24),
                  SizedBox(
                    width: double.infinity,
                    child: _ProfileSectionCard(
                      ...
                      rows: [_InfoRowData('Ruolo', _getAdminRoleText(person))],
                    ),
                  ),
                ],

                if (rawRoles.contains('DOCENTE') ||
                    rawRoles.contains('TEACHER')) ...[
                  const SizedBox(height: 24),
                  SizedBox(
                    width: double.infinity,
                    child: _ProfileSectionCard(
                      ...
                      rows: [
                        _InfoRowData(
                          'Studi scolastici',
                          person.schoolEducation?.isNotEmpty == true
                              ? person.schoolEducation!
                              : '-',
                        ),
                        _InfoRowData(
                          'Studi universitari',
                          person.universityEducation?.isNotEmpty == true
                              ? person.universityEducation!
                              : '-',
                        ),
                      ],
                    ),
                ...
                if (rawRoles.contains('STUDENTE') ||
                    rawRoles.contains('STUDENT')) ...[
                  const SizedBox(height: 24),
                  SizedBox(
                    width: double.infinity,
                    child: _ProfileSectionCard(
                ...
                if (rawRoles.contains('CORSISTA') ||
                    rawRoles.contains('COURSE_PARTICIPANT')) ...[
                  const SizedBox(height: 24),
                  SizedBox(
                    width: double.infinity,
                    child: _ProfileSectionCard(
                      ...
                      rows: [
                        _InfoRowData(
                          'Tipo corso',
                          person.courseType?.isNotEmpty == true
                              ? person.courseType!
                              : '-',
                        ),
                        _InfoRowData(
                          'Scadenza cert. medico',
                          person.medicalCertificateExpiration != null
                              ? DateFormat(
                                  'dd/MM/yyyy',
                                ).format(person.medicalCertificateExpiration!)
                              : '-',
                        ),
                      ],
                    ),
    ...
    for (int i = 0; i < rows!.length; i++) {
      ...
      if (rowData == null) {
        rowWidget = const Opacity(
          opacity: 0.0,
          child: _ProfileInfoRow(label: '-', value: '-'),
        );
      } else {
        rowWidget = _ProfileInfoRow(label: rowData.label, value: rowData.value);
      }
...
class _ProfileAvatar extends StatefulWidget {
  ...
  const _ProfileAvatar({required this.onImageUpdated, this.profileImageUrl});
