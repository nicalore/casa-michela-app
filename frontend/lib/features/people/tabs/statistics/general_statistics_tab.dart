...
class _GeneralStatisticsTabState extends State<GeneralStatisticsTab> 
{
  ...
  //Comment:Generate a filled sequential list including zeros for missing periods
  ...
  {
    return [_StatFilterOption(value: 'year', label: 'Annuale'), _StatFilterOption(value: 'month', label: 'Mensile')];
  }
  ...
  List<_StatFilterOption<int>> _getYearOptions() 
  {
    ...
    return List.generate(currentYear - 2022 + 1, (index) => currentYear - index).map((y) => _StatFilterOption(value: y, label: y.toString())).toList();
  }
  ...
  List<_StatFilterOption<int>> _getCollabMonthOptions() 
  {
    ...
    {
      return [_StatFilterOption(value: 11, label: 'Nov'), _StatFilterOption(value: 12, label: 'Dic')];
    }
      ...
      child: Column(
        ...
        [
          Row(
            ...
            children: 
            [
              Text('Fidelizzazione Collaboratori Attivi', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
              Wrap(
                spacing: 12,
                children: 
                [
                  _StatFilterMenu<String>(
                    hint: 'Tipo', value: _collabRetentionType, options: _getRetentionTypeOptions(),
                    onChanged: (v) 
                    { 
                      setState(() 
                      { 
                  ...
                  if (_collabRetentionType == 'month')
                    _StatFilterMenu<int>(
                      hint: 'Mese', value: _selectedCollabMonth, options: _getCollabMonthOptions(),
                      onChanged: (v) { setState(() => _selectedCollabMonth = v); _loadData(); },
                    ),
                  _StatFilterMenu<int>(
                    hint: 'Anno', value: _selectedCollabYear, options: _getYearOptions(),
                    onChanged: (v) 
                    { 
                      setState(() 
                      { 
      ...
      child: Column(
        ...
        [
            ...
            [
              ...
              if (showFilters)
                Wrap(
                  ...
                  children: 
                  [
                    _StatFilterMenu<String>(hint: 'Risoluzione', value: resolution, options: _getResolutionOptions(), onChanged: onResolutionChanged),
                    _StatFilterMenu<int>(hint: 'Da anno', value: startYear, options: _getYearOptions(), onChanged: onStartYearChanged),
                    _StatFilterMenu<int>(hint: 'A anno', value: endYear, options: _getYearOptions(), onChanged: onEndYearChanged),
                  ],
                )
            ],
          ),
          ...
          SizedBox(
            height: 280,
            child: data.isEmpty 
              ? Center(child: Text('Nessun dato', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 16)))
              : _LineChartWidget(data: data, isMonthly: isMonthly),
          ),
        ],
      ),
    );
...
class _LineChartWidgetState extends State<_LineChartWidget> 
{
  ...
  Widget build(BuildContext context) 
  {
    ...
    //Comment:Enforce gridmaxValue to be a multiple of 4 to prevent overlapping fractions
          ...
          [
            Positioned.fill(
              child: MouseRegion(
                ...
                child: CustomPaint(
                  size:    Size.infinite, 
                  painter: _LineChartPainter(data: widget.data, maxValue: maxValue, isMonthly: widget.isMonthly, hoverPosition: _hoverPosition)
                ),
              ),
            ),
...
{ 
  final T      value; 
  final String label; 
  _StatFilterOption({required this.value, required this.label}); 
}
...
{ 
  ...
  const _StatFilterMenu({required this.hint, required this.value, required this.options, required this.onChanged}); 
...
{ 
  ...
  { 
    ...
    //Comment:Convert coordinates based on the root overlay to avoid nested navigator shifts
        ...
        [
          ...
          Positioned(
            ...
            child: _StatFilterOverlayContent<T>( // O _OverlayContent in role_specific_statistics_view
              ...
              onSelected:   (val) { widget.onChanged(val); _closeMenu(); }
            )
          )
        ]
      )
    ); 
    //Comment:Use rootOverlay to ensure correct screen-space alignment
  ...
  Widget build(BuildContext context) 
  { 
    final selectedOption = widget.options.firstWhere((o) => o.value == widget.value, orElse: () => _StatFilterOption(value: widget.value, label: '')); 
...
{ 
  ...
  const _StatFilterOverlayContent({super.key, required this.currentValue, required this.options, required this.onSelected}); 
            ...
            ? Padding(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    mainAxisSize:       MainAxisSize.min, 
                    crossAxisAlignment: CrossAxisAlignment.start, 
                    children:           widget.options.map((option) { return _StatFilterMenuItem(text: option.label, isSelected: widget.currentValue == option.value, onTap: () => widget.onSelected(option.value)); }).toList()
                  )
                )
              ) 
...
{ 
  ...
  const _StatFilterMenuItem({required this.text, required this.isSelected, required this.onTap}); 
