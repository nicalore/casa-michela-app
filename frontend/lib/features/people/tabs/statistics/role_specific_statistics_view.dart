...
class ChartBarItem 
{
  final String label;
  final int    count;
  ChartBarItem({required this.label, required this.count});
}
...
class ChartPieItem 
{
  final String label;
  final int    count;
  ChartPieItem({required this.label, required this.count});
}
...
class RoleSpecificStatisticsView extends StatefulWidget 
{
  ...
  const RoleSpecificStatisticsView({required this.roleKey, super.key});
...
class _RoleSpecificStatisticsViewState extends State<RoleSpecificStatisticsView> 
{
  ...
  static const List<String> _months = ['Gen', 'Feb', 'Mar', 'Apr', 'Mag', 'Giu', 'Lug', 'Ago', 'Set', 'Ott', 'Nov', 'Dic'];
  ...
  {
    if (widget.roleKey != 'course_participant') return;
    try 
    {
        ...
        [
          Row(
            ...
            [
              Container(
                ...
                decoration: const BoxDecoration(color: Color(0xFFF5F8FC), shape: BoxShape.circle), 
                child:      Icon(icon, color: const Color(0xFF003C82), size: 24)
              ),
              const SizedBox(width: 12),
              Text(
                title, 
                style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w700, color: const Color(0xFF1E293B)),
              ),
            ],
          ),
          ...
          Row(
            ...
            [
              ...
              Container(width: 1, height: 45, color: const Color(0xFFE2E8F0), margin: const EdgeInsets.symmetric(horizontal: 16)),
  ...
  Widget _buildTrendChartCard({required String title, required List<MemberTrendItem> data, required bool isMonthly, required bool showFilters, required String resolution, required int startYear, required int endYear, required ValueChanged<String> onResolutionChanged, required ValueChanged<int> onStartYearChanged, required ValueChanged<int> onEndYearChanged}) 
  {
    return Container(
      padding:    const EdgeInsets.all(24),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: const [BoxShadow(color: Color(0x0A000000), offset: Offset(0, 4), blurRadius: 16)]),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: 
        [
          Row(
            ...
            children: 
            [
              Text(title, style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
              if (showFilters)
                Wrap(
                  spacing: 12,
                  children: 
                  [
                    _FilterMenuWidget<String>(hint: 'Risoluzione', value: resolution, options: _getResolutionOptions(), onChanged: onResolutionChanged),
                    _FilterMenuWidget<int>(hint: 'Da anno', value: startYear, options: _getYearOptions(), onChanged: onStartYearChanged),
                    _FilterMenuWidget<int>(hint: 'A anno', value: endYear, options: _getYearOptions(), onChanged: onEndYearChanged),
                  ],
                ),
            ],
          ),
          const SizedBox(height: 48),
          SizedBox(height: 280, child: data.isEmpty ? Center(child: Text('Nessun dato', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 16))) : _LineChartWidget(data: data, isMonthly: isMonthly)),
        ],
      ),
    );
  }
  ...
  Widget _buildCityChartCard() 
  {
    return Container(
      ...
      child: Column(
        ...
        children: 
        [
          Text('Distribuzione per Città', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
          const SizedBox(height: 48),
          SizedBox(
            height: 280, 
            child:  _cityData.isEmpty ? Center(child: Text('Nessun dato', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 16))) 
            : _BarChartWidget(data: _cityData.map((e) => ChartBarItem(label: e.city, count: e.count)).toList())
          ),
        ],
      ),
    );
  ...
  Widget _buildAgeChartCard() 
  {
    return Container(
      ...
      child: Column(
        ...
        children: 
        [
          Text('Fasce d\'età', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
          const SizedBox(height: 32),
          SizedBox(
            height: 296, 
            child:  _ageData.isEmpty ? Center(child: Text('Nessun dato', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 16))) 
            : _PieChartWidget(data: _ageData.map((e) => ChartPieItem(label: e.ageGroup, count: e.count)).toList())
          ),
        ],
      ),
    );
      ...
      child: Column(
        ...
        [
          Row(
            ...
            children: 
            [
              Text('Distribuzione Scolastica', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
              _FilterMenuWidget<String>(
                ...
                onChanged: (v) { setState(() => _educationDistributionType = v); _loadEducationData(); }
              ),
            ],
          ),
          const SizedBox(height: 48),
          SizedBox(
            height: 280, 
            child:  _educationData.isEmpty ? Center(child: Text('Nessun dato', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 16))) 
            : _BarChartWidget(data: _educationData.map((e) => ChartBarItem(label: e.label, count: e.count)).toList())
          ),
        ],
      ),
    );
  ...
  Widget _buildCourseChartCard() 
  {
    return Container(
      ...
      child: Column(
        ...
        children: 
        [
          Text('Distribuzione per Corso', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
          const SizedBox(height: 48),
          SizedBox(
            height: 280, 
            child:  _courseData.isEmpty ? Center(child: Text('Nessun dato', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 16))) 
            : _BarChartWidget(data: _courseData.map((e) => ChartBarItem(label: e.label, count: e.count)).toList())
          ),
        ],
      ),
    );
  ...
  Widget _buildSubjectRow(SubjectDistributionItem s) 
  {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: 
        [
          Expanded(child: Text(s.name, style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w500, color: const Color(0xFF64748B)), overflow: TextOverflow.ellipsis)),
          const SizedBox(width: 12),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
            decoration: BoxDecoration(color: const Color(0xFFE0F2FE), borderRadius: BorderRadius.circular(12)),
            child: Text('${s.count} $label', style: GoogleFonts.plusJakartaSans(fontSize: 12, fontWeight: FontWeight.w700, color: const Color(0xFF0284C7))),
          ),
        ],
      ),
    );
  ...
  {
    if (_teacherStats == null) return const SizedBox();
      ...
      child: Column(
        ...
        [
          Row(
            ...
            children: 
            [
              Text('Analisi Competenze e Insegnamenti', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w700, color: const Color(0xFF1E293B))),
              _FilterMenuWidget<String>(
                hint: 'Classifica', 
                ...
                onChanged: (v) { setState(() => _teacherRankingMode = v); _loadTeacherData(); }
              ),
            ],
          ),
          const SizedBox(height: 32),
            ...
            [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Media discipline per docente', style: GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w600, color: const Color(0xFF64748B))),
                    const SizedBox(height: 8),
                    Text(_teacherStats!.avgSubjectsPerTeacher.toStringAsFixed(1), style: GoogleFonts.plusJakartaSans(fontSize: 36, fontWeight: FontWeight.w800, color: const Color(0xFF003C82))),
                  ],
                ),
              ),
              Container(width: 1, height: 45, color: const Color(0xFFE2E8F0), margin: const EdgeInsets.symmetric(horizontal: 16)),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Media docenti per disciplina', style: GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w600, color: const Color(0xFF64748B))),
                    const SizedBox(height: 8),
                    Text(_teacherStats!.avgTeachersPerSubject.toStringAsFixed(1), style: GoogleFonts.plusJakartaSans(fontSize: 36, fontWeight: FontWeight.w800, color: const Color(0xFF003C82))),
                  ],
                ),
              ),
            ],
              ...
              [
                Expanded(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Top 10 - Più insegnate', style: GoogleFonts.plusJakartaSans(fontSize: 16, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
                      const SizedBox(height: 24),
                      if (_teacherStats!.top10Subjects.isEmpty)
                        Text('Nessun dato', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 14))
                      else
                ...
                Expanded(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Bottom 10 - Meno insegnate', style: GoogleFonts.plusJakartaSans(fontSize: 16, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
                      const SizedBox(height: 24),
                ...
                Expanded(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('Distribuzione per Area', style: GoogleFonts.plusJakartaSans(fontSize: 16, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
                      const SizedBox(height: 24),
                      SizedBox(
                        height: 320, 
                        child:  _teacherStats!.areaDistribution.isEmpty ? Center(child: Text('Nessun dato', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 14))) 
                        : _PieChartWidget(data: _teacherStats!.areaDistribution.map((e) => ChartPieItem(label: _translateArea(e.area), count: e.count)).toList())
                      ),
                    ],
                  ),
                ),
          ...
          child: Column(
            ...
            [
                      ...
                      child: Column(
                        ...
                        [
                          Row(
                            ...
                            children: 
                            [
                              Text('Fidelizzazione Ruolo (Annuale)', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
                              _FilterMenuWidget<int>(hint: 'Anno', value: _selectedRetentionYear, options: _getYearOptions(), onChanged: (v) { setState(() => _selectedRetentionYear = v); _loadData(); }),
                            ],
                          ),
                          const SizedBox(height: 24),
                      ...
                      child: Column(
                        ...
                        [
                          Row(
                            ...
                            children: 
                            [
                              Text('Fidelizzazione Collaboratori Ruolo', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
                              Wrap(
                                spacing: 12,
                                children: 
                                [
                                  _FilterMenuWidget<String>(hint: 'Tipo', value: _collabRetentionType, options: [_FilterMenuOption(value: 'year', label: 'Annuale'), _FilterMenuOption(value: 'month', label: 'Mensile')], onChanged: (v) { setState(() { _collabRetentionType = v; if (v == 'month' && _selectedCollabYear == 2022 && _selectedCollabMonth < 11) _selectedCollabMonth = 11; }); _loadData(); }),
                                  if (_collabRetentionType == 'month') _FilterMenuWidget<int>(hint: 'Mese', value: _selectedCollabMonth, options: _getCollabMonthOptions(), onChanged: (v) { setState(() => _selectedCollabMonth = v); _loadData(); }),
                                  _FilterMenuWidget<int>(hint: 'Anno', value: _selectedCollabYear, options: _getYearOptions(), onChanged: (v) { setState(() { _selectedCollabYear = v; if (v == 2022 && _selectedCollabMonth < 11) _selectedCollabMonth = 11; }); _loadData(); }),
                                ],
                              ),
                            ],
                          ),
                          ...
                          if (_collabRetentionData != null)
                            Row(
                              children: 
                              [
                                Text('${_collabRetentionData!.retentionRatePercentage.toStringAsFixed(1)}%', style: GoogleFonts.plusJakartaSans(fontSize: 54, fontWeight: FontWeight.w700, color: const Color(0xFF003C82))),
                                const SizedBox(width: 16),
                                Expanded(
                                  child: Text(
              ...
              _buildTrendChartCard(
                ...
                onResolutionChanged: (v) { setState(() => _trendResolution = v); _loadData(); }, 
                onStartYearChanged:  (v) { setState(() { _startTrendYear = v; if (_endTrendYear < v) _endTrendYear = v; }); _loadData(); }, 
                onEndYearChanged:    (v) { setState(() { _endTrendYear = v; if (_startTrendYear > v) _startTrendYear = v; }); _loadData(); }
              ),
              const SizedBox(height: 24),
              _buildTrendChartCard(
                ...
                onResolutionChanged: (v) { setState(() => _collabTrendResolution = v); _loadData(); }, 
                onStartYearChanged:  (v) { setState(() { _startCollabTrendYear = v; if (_endCollabTrendYear < v) _endCollabTrendYear = v; }); _loadData(); }, 
                onEndYearChanged:    (v) { setState(() { _endCollabTrendYear = v; if (_startCollabTrendYear > v) _startCollabTrendYear = v; }); _loadData(); }
              ),
...
class _LineChartWidget extends StatefulWidget 
{
  ...
  const _LineChartWidget({required this.data, required this.isMonthly});
            ...
            Positioned.fill(
              child: MouseRegion(
                onHover: (event) 
                {
                  ...
                  setState(() { _hoverPosition = pos; _hoveredIndex = foundIndex; if (foundIndex != null) { _popupTargetPosition = points[foundIndex]; _cachedMembersValue = widget.data[foundIndex].totalMembers; } });
                },
                onExit: (_) => setState(() { _hoverPosition = null; _hoveredIndex = null; }),
                child: CustomPaint(size: Size.infinite, painter: _LineChartPainter(data: widget.data, maxValue: maxValue, isMonthly: widget.isMonthly, hoverPosition: _hoverPosition)),
              ),
            ),
            if (_popupTargetPosition != null)
              Positioned(
                left: _popupTargetPosition!.dx, top: _popupTargetPosition!.dy - 10,
                child: IgnorePointer(
                  child: FractionalTranslation(
                    translation: const Offset(-0.5, -1.0),
                    child: AnimatedScale(
                      scale: _hoveredIndex != null ? 1.0 : 0.6, duration: const Duration(milliseconds: 200), curve: Curves.easeOutBack,
                      child: AnimatedOpacity(
                        opacity: _hoveredIndex != null ? 1.0 : 0.0, duration: const Duration(milliseconds: 150), curve: Curves.easeOut,
                        child: Column(
                        ...
                        child: MouseRegion(
                          onHover: (event) 
                          {
                            ...
                            { 
                              ...
                              if (foundIndex != null) { _popupTargetPosition = Offset(barRects[foundIndex].center.dx, barRects[foundIndex].top); _cachedCountValue = widget.data[foundIndex].count; _cachedLabelName = widget.data[foundIndex].label; } 
                            });
                          },
                          onExit: (_) => setState(() { _hoverPosition = null; _hoveredIndex = null; }),
                          child: CustomPaint(size: Size.infinite, painter: _BarChartPainter(data: widget.data, maxValue: maxValue, hoverPosition: _hoverPosition, barRects: barRects)),
                        ),
                      ),
                      if (_popupTargetPosition != null)
                        Positioned(
...
class _YAxisPainter extends CustomPainter 
{
  ...
  _YAxisPainter({required this.maxValue, required this.paddingBottom});
  ...
  void paint(Canvas canvas, Size size) 
  {
    ...
    for (int i = 0; i <= 4; i++) 
    {
      ...
      textPainter.text = TextSpan(text: '${((maxValue / 4) * i).round()}', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 13));
      textPainter.layout();
...
class _BarChartPainter extends CustomPainter 
{
  ...
  _BarChartPainter({required this.data, required this.maxValue, this.hoverPosition, required this.barRects});
  ...
  void paint(Canvas canvas, Size size) 
  {
    ...
    for (int i = 0; i < barRects.length; i++) 
    {
      ...
      textPainter.text = TextSpan(text: data[i].label, style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 13, fontWeight: FontWeight.w600));
          ...
          [
            Positioned.fill(
              child: MouseRegion(
                ...
                onExit: (_) => setState(() { _hoverPosition = null; _hoveredIndex = null; }),
                child: CustomPaint(size: Size.infinite, painter: _PieChartPainter(data: widget.data, total: total, colors: _colors, center: center, radius: radius, hoveredIndex: _hoveredIndex)),
              ),
            ),
            if (_popupTargetPosition != null)
              Positioned(
                left: _popupTargetPosition!.dx, top: _popupTargetPosition!.dy - 15,
                child: IgnorePointer(
                  child: FractionalTranslation(
                    translation: const Offset(-0.5, -1.0),
                    child: AnimatedScale(
                        ...
                        child: Column(
                          ...
                          [
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), 
                              decoration: BoxDecoration(color: const Color(0xFF1E293B), borderRadius: BorderRadius.circular(8), boxShadow: const [BoxShadow(color: Color(0x1F000000), offset: Offset(0, 3), blurRadius: 6)]), 
                              child: Text('${_cachedLabel ?? ""}: ${_cachedCount ?? 0} (${_cachedPercentage?.toStringAsFixed(1) ?? "0"}%)', style: GoogleFonts.plusJakartaSans(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w700))
                            ),
                            CustomPaint(size: const Size(10, 5), painter: _TriangleArrowPainter(color: const Color(0xFF1E293B))),
                          ],
                        ),
                    ...
                    return Padding(
                      ...
                      child: Row(
                        ...
                        [
                          ...
                          const SizedBox(width: 12),
                          Text('${widget.data[i].label} ', style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
                          Text('(${p.toStringAsFixed(1)}%)', style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w500, color: const Color(0xFF64748B))),
                        ],
                      ),
                    );
                  }),
...
class _PieChartPainter extends CustomPainter 
{
  ...
  _PieChartPainter({required this.data, required this.total, required this.colors, required this.center, required this.radius, this.hoveredIndex});
  ...
  void paint(Canvas canvas, Size size) 
  {
    ...
    for (int i = 0; i < data.length; i++) 
    {
      ...
      canvas.drawArc(Rect.fromCircle(center: center, radius: radius), startAngle + gap / 2, math.max(0, sweepAngle - gap), false, paint);
      startAngle += sweepAngle;
    }
  }
...
class _LineChartPainter extends CustomPainter 
{
  ...
  static const List<String> _monthNames = ['Gen', 'Feb', 'Mar', 'Apr', 'Mag', 'Giu', 'Lug', 'Ago', 'Set', 'Ott', 'Nov', 'Dic'];

  _LineChartPainter({required this.data, required this.maxValue, required this.isMonthly, this.hoverPosition});

  @override
  void paint(Canvas canvas, Size size) 
  {
    if (data.isEmpty) return;
    final paintLine = Paint()..color = const Color(0xFF003C82)..strokeWidth = 3..style = PaintingStyle.stroke..strokeCap = StrokeCap.round..strokeJoin = StrokeJoin.round;
    final paintGrid = Paint()..color = const Color(0xFFE2E8F0)..strokeWidth = 1..style = PaintingStyle.stroke;
    final textPainter = TextPainter(textDirection: TextDirection.ltr);
    ...
    for (int i = 0; i < data.length; i++) 
    {
      ...
      textPainter.text = TextSpan(text: isMonthly ? '${_monthNames[(data[i].month ?? 1) - 1]}\n${data[i].year}' : '${data[i].year}', style: GoogleFonts.plusJakartaSans(color: const Color(0xFF94A3B8), fontSize: 13));
      textPainter.textAlign = TextAlign.center;
      textPainter.layout();
    ...
    final fillPath = Path.from(path)..lineTo(points.last.dx, chartHeight)..lineTo(points.first.dx, chartHeight)..close();
    canvas.drawPath(fillPath, Paint()..shader = LinearGradient(colors: [const Color(0x33003C82), const Color(0x00003C82)], begin: Alignment.topCenter, end: Alignment.bottomCenter).createShader(Rect.fromLTWH(paddingLeft, 0, chartWidth, chartHeight))..style = PaintingStyle.fill);
...
class _TriangleArrowPainter extends CustomPainter 
{
  ...
  void paint(Canvas canvas, Size size) 
  {
    canvas.drawPath(Path()..moveTo(0, 0)..lineTo(size.width, 0)..lineTo(size.width / 2, size.height)..close(), Paint()..color = color..style = PaintingStyle.fill);
  }
  @override
  bool shouldRepaint(covariant CustomPainter oldDelegate) => false;
}

class _FilterMenuOption<T> { final T value; final String label; _FilterMenuOption({required this.value, required this.label}); }
...
{ 
  ...
  const _FilterMenuWidget({required this.hint, required this.value, required this.options, required this.onChanged}); 
  @override State<_FilterMenuWidget<T>> createState() => _FilterMenuWidgetState<T>(); 
}
...
{ 
  ...
  { 
    ...
    _overlayEntry = OverlayEntry(builder: (context) => Stack(children: [Positioned.fill(child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: _closeMenu, child: Container())), Positioned(top: offset.dy + size.height + 8, left: offset.dx, child: _OverlayContent<T>(key: _menuKey, currentValue: widget.value, options: widget.options, onSelected: (val) { widget.onChanged(val); _closeMenu(); }))])); Overlay.of(context, rootOverlay: true).insert(_overlayEntry!); 
  } 
  ...
  Widget build(BuildContext context) 
  { 
    final selectedOption = widget.options.firstWhere((o) => o.value == widget.value, orElse: () => _FilterMenuOption(value: widget.value, label: '')); 
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(color: _isHovered ? const Color(0xFFF5F8FC) : Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: _isHovered ? const Color(0xFF003C82) : const Color(0xFFE0E5EC), width: 1.5), boxShadow: const [BoxShadow(color: Color(0x05000000), offset: Offset(0, 2), blurRadius: 8)]), 
          child: Row(
            mainAxisSize: MainAxisSize.min, 
            children: 
            [
              Text('${widget.hint}: ', style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w500, color: const Color(0xFF8A8A8A))), 
              Text(selectedOption.label, style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w600, color: const Color(0xFF003C82))), 
              const SizedBox(width: 8), 
              AnimatedRotation(turns: _isMenuOpen ? 0.5 : 0.0, duration: const Duration(milliseconds: 200), curve: Curves.easeInOut, child: const Icon(Icons.keyboard_arrow_down_rounded, color: Color(0xFF003C82), size: 18))
            ]
          )
        )
      )
...
{ 
  ...
  const _OverlayContent({super.key, required this.currentValue, required this.options, required this.onSelected}); 
  @override State<_OverlayContent<T>> createState() => _OverlayContentState<T>(); 
}
            ...
            ? Padding(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    mainAxisSize: MainAxisSize.min, 
                    crossAxisAlignment: CrossAxisAlignment.start, 
                    children: widget.options.map((option) { return _ItemWidget(text: option.label, isSelected: widget.currentValue == option.value, onTap: () => widget.onSelected(option.value)); }).toList()
                  )
                )
              ) 
...
{ 
  ...
  const _ItemWidget({required this.text, required this.isSelected, required this.onTap}); 
  @override State<_ItemWidget> createState() => _ItemWidgetState(); 
}
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Row(
            children: 
            [
              AnimatedContainer(duration: const Duration(milliseconds: 150), width: 2, height: (_hover || widget.isSelected) ? 16 : 0, decoration: BoxDecoration(color: const Color(0xFF003C82), borderRadius: BorderRadius.circular(2))), 
              const SizedBox(width: 10), 
              Expanded(child: Text(widget.text, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: widget.isSelected ? FontWeight.w700 : FontWeight.w500, color: const Color(0xFF003C82))))
            ]
          )
        )
      )
...
class AntiquityHelper
{
  ...
  {
    final Map<String, int> order = { "< 11": 1, "11-14": 2, "15-18": 3, "19-25": 4, "26-35": 5, "36-50": 6, "> 50": 7 };
